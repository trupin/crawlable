...
Client.prototype.start = function (callback) {
    if (!this.options.server) {
        this._mq.on(this._id, this.onTaskResponse.bind(this));
        this._mq.start(callback);
    }
    else callback(null);
};

Client.prototype.onTaskResponse = function (err, task, next) {
    if (!task) return;
...
Client.prototype.exec = function (task, callback) {
    ...
    if (this.options.server)
        this.options.server.onTask(task);
    else
 ...
 * @param callback
 */
Client.prototype.getStaticHtml = function (options, callback) {
    var that = this;
    ...
    async.waterfall([
        function (next) {
            ...
            return db.pages.get(options.pathname, function (err, model) {
                next(null, model || null);
            });
        },
        function (model, next) {
            if (!model || model.get('luts') + that._ttl < +new Date()) {
                that.forceComputeCache(options.pathname, options.waitForRegeneration ? next : function (err) {
                    if (err) return console.log(err);
                    console.log('Page cache has been regenerated.');
                });
                if (options.waitForRegeneration) return;
            }
            callback(!model ? new Error("No cache at the moment.") : null, model);
        },
        function (next) {
            console.log('Page cache has been regenerated');
            db.pages.get(options.pathname, next);
        }
    ], callback);
};