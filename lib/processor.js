...
var _ = require('lodash'),
    async = require('async'),
    Solidify = require('crawlable-solidify'),
...
var errors = require('./errors.js'),
    ...
    DefaultRenderer = require('./renderer/waitFor.js'),
    DefaultPersistence = require('./persistence/nedb.js');
...
var Processor = module.exports = function (options) {
    ...
    if (!options.logger) {
        ...
        this.logger.add(winston.transports.Console, {
            level: _.isString(options.logLevel) ? options.logLevel : 'info',
...
Processor.prototype.start = function (callback) {
    ...
    phOpts.onCriticalExit = function () {
        that.logger.warn('PhantomJS exited with an abnormal error code and will be relaunched.');
        ...
        that._setUpPhantomJs(phOpts, function (err) {
            if (err) {
                that._failure = true;
                return that.logger.errors('Couldn\'t restart PhantomJs after it crashed.');
            }
            that.logger.warn('PhantomJS restarted.');
...
Processor.prototype.onCacheActualize = function (data) {
    if (data.luts + this.options.cacheTtl < +new Date()) {
        ...
        this._tasksQueue.push(Processor._models.task(data._id, pathname, data, function (err) {
            if (err)
                return this.logger.error('Couldn\'t actualize the cache entry for route: "%s".', data.route);
            this.logger.info('The cache entry with the route "%s" has been actualized.', data.route);
        }));
    }
};
...
Processor.prototype.crawl = function (callback) {
    ...
    async.forEachLimit(that._router.routes, that.options.concurrency, function (route, done) {
        ...
        that.call(pathname, { force: true, wait: true }, function (err) {
            if (err)
                return done(err);
            that.logger.info('[%s] Crawled', route.regexp.toString().length > 20 ?
                (route.regexp.toString().substring(0, 20) + '...') : route.regexp.toString());
            done(null);
        });
    }, callback);
};
...
Processor.prototype.express = function (options) {
    ...
    return function (req, res, next) {
        ...
        that.call(req._parsedUrl.pathname, opts, function (err, page) {
            if (err) {
                that.logger.error('[pathname="%s"] %s', req._parsedUrl.pathname, err.message);
                req.crawlable.html = '';
            }
            else {
                req.crawlable.html = page.html;
