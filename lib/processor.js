    ...
    function (context, next) {
        if (context.options.force || !context.cached.template) {
            this._tasksQueue.push(Processor._models.task(
                context.cached._id, context.cached, !context.options.wait ? null :
                    function (err, cached) {
...
Processor.prototype.start = function (callback) {
    ...
    that._tasksQueue.push = function (task) {
        var eqTask = _.find(this.tasks, { routeId: task.routeId });
        if (eqTask)
...
Processor.prototype._render = function (cached, callback) {
    ...
    async.waterfall([
        ...
        function (rawTemplate, next) {
            that._phantomPages.push(phPage);
        ...
        that.call(req._parsedUrl.pathname, opts, function (err, page) {
            if (err instanceof errors.NotFound) {
                if (options.onNotFound)
                    options.onNotFound(err, res, next);
