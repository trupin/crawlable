...
//            requests: []
     ...
     * @returns {object} task
     */
    task: function (routeId, cached, callback) {
        return {
            routeId: routeId,
...
Processor._middlewares = {
    ...
    renderPageIfNecessary: function (context, next) {
        if (context.options.force || !context.cached.template || context.cached.error)
            this._tasksQueue.push(Processor._models.task(
                context.cached._id, context.cached, context.options.wait ? next : null
            ));
        else next(null);
    },
    /**
    ...
    onPageRendered: function (context, next) {
        ...
        return this._solidify.feed(opts, function (err, result) {
            if (err)
                return next(new error.Internal('[Crawlable] Solidify error: ' + err.message));
...
Processor.prototype.start = function (callback) {
    ...
    that._tasksQueue = async.queue(function (task, callback) {
        ...
        that._render(task.cached, function (err, res) {
    ...
    phOpts.onCriticalExit = function () {
        ...
        _.each(that._tasksInProcess, function (task) {
 ...
 * @private
 */
Processor.prototype._render = function (cached, callback) {
    var that = this;

    var phPage = that._phantomPages.shift(),
        url = that.options.host + cached.route;
    ...
    async.waterfall([
        ...
        function (next) {
            that._renderer.run(phPage, next);
        },
        function (rawTemplate, next) {
            that._solidify.compile(rawTemplate, next);
        },
        function (rendered, next) {
            if (!rendered)
...
Processor.prototype.onCacheActualize = function (data) {
    if (data.luts + this.options.cacheTtl < +new Date())
        this._tasksQueue.push(Processor._models.task(data._id, data, function (err) {
            if (err)
...
Processor.prototype.crawl = function (callback) {
    ...
    async.forEachLimit(that._router.routes, that.options.concurrency, function (route, done) {
        ...
        that.call(pathname, { force: true, wait: true }, function (err) {
           if (err)
               console.log('[Crawlable] An error occured for the route "' + route + '", crawled with the pathname "' + pathname + '".');
           done(null);
        });
    }, callback);
};
